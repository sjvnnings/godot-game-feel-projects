...
onready var cam := $shakeable_camera/Camera as Camera
onready var rotator := $shakeable_camera
...
func _input(event):
	...
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED:
		...
		rotator.rotation_degrees.x = clamp(rotator.rotation_degrees.x + delta_x, min_x_rot, max_x_rot)
